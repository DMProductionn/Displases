...
export default function FilterBtn() {
  ...
  const { filterMenu, colorBtn, priceBtn } = useSelector((state) => state.Filter);
  ...
  return (
    <div className={`flex relative`}>
      ...
      <button
      onClick={showFilter}
        className={ filterMenu ? `${style.filterBtn} ${style.active}` : `${style.filterBtn}`}>
        ФИЛЬТРЫ
        <svg
      ...
      <div
        className={ filterMenu ? `${style.active} ${style.FilterPopup} ` : `${style.FilterPopup}`}>
        ...
        {/* фильтры цены */}
        <div className={ priceBtn ? `${style.price} ${style.active}` : `${style.price}`}>
          <ul>
            <li className={`py-[6px]`}>
        ...
        {/* фильтры цветов */}
        <div className={colorBtn ? `${style.color} ${style.active}`: `${style.color}`}>
          <ul>
            <li className={`py-[6px]`}>
